# 🔗 命名规范与接口定义 (Interface Definition)
...
## 3. 驱动模块配置参数 (Driver Configuration)

### 3.1 MPU6050 (IMU)

| 参数 | 值 | 说明 |
|------|-----|------|
| **通信接口** | I2C1 | 400kHz Fast Mode |
| **陀螺仪量程** | ±2000 dps | 高动态范围 |
| **加速度计量程** | ±8g | 适合穿越机 |
| **采样率** | 1kHz | SMPLRT_DIV = 0 |
| **DLPF** | 42Hz | 平衡延迟与噪声 |
| **驱动文件** | `mpu6050.c`, `mpu6050.h` | - |
| **关键函数** | `MPU6050_Read_All_Calibrated()` | - |

### 3.2 SPL06-001 (气压计)

| 参数 | 值 | 说明 |
|------|-----|------|
| **通信接口** | I2C1 | 与 MPU6050 共享 |
| **气压采样率** | 32Hz | 16x 过采样 |
| **温度采样率** | 32Hz | 1x 过采样 |
| **驱动文件** | `spl06.c`, `spl06.h` | - |
| **关键函数** | `SPL06_ReadRawData()` | - |
...
| **采样率** | 50Hz | 连续测量模式 |
| **量程** | ±2 Gauss | 默认量程 |
| **驱动文件** | `qmc5883l.c`, `qmc5883l.h` | - |
| **关键函数** | `QMC5883L_ReadRaw()`, `QMC5883L_CalculateHeading()` | - |
...
| **通信接口** | SPI1 | 10.5MHz |
| **视频制式** | PAL/NTSC | 自动检测 |
| **刷新率** | 10Hz | 在 comm_task 中更新 |
| **驱动文件** | `at7456e.c`, `at7456e.h` | - |
| **关键函数** | `AT7456E_Update()` | - |
...
| 参数 | 值 | 说明 |
|------|-----|------|
| **通信接口** | USART2 | 115200 baud |
| **协议** | NMEA 0183 | GNGGA, GNRMC |
| **驱动文件** | `gps.c`, `gps.h` | - |
| **关键函数** | `GPS_Parse_NMEA()` | - |
...
## 5. 外设句柄命名规范 (Peripheral Handles) - STM32F405RGT6
...
| 2026-01-22 | 文档整合: 重命名为 01_definitions.md | Kilo Code |
| 2026-01-22 | 合并驱动模块配置参数 (从 04_Module_Drivers_Deep_Dive.md) | Kilo Code |

---

## 8. 冲突检查清单 (Conflict Checklist)
...
- [ ] 是否需要更新 [`08_hardware_design.md`](08_hardware_design.md)?
...
3. 修改结构体定义时,**必须**同步更新 [`04_algo_design.md`](04_algo_design.md)
4. 发现重复定义时,立即报告并在 [`03_issue_tracker.md`](03_issue_tracker.md) 中记录